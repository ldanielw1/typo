...
class Admin::ContentController < Admin::BaseController
  ...
  def merge_with
    if current_user.admin?
      first_article = Article.find_by_id(params[:article_id])
      other_article = Article.find_by_id(params[:merge_with])
      result = first_article.merge_with(other_article)
      flash[:error] = "Unsuccessful merge: Attempted to merge with non-existent article ID" if result == nil
